...
<head>
    ...
    <title>Data User Sema & Dema</title>
...
$insert = "SELECT nama from user where nama='$nama' and keterangan='Sema & Dema'";
$rule = $db->prepare($insert);
$rule->setFetchMode(PDO::FETCH_OBJ);
$rule->execute();

$insert = "SELECT username from user where username='$username' and keterangan='Sema & Dema'";
...
//Row will return false if there was no value
        if ($rule->rowCount() == 0 && $stmti->rowCount() == 0) {
            //insert new data
            $insert = "insert into user values(' ','$nama', 'Tidak Ada',' ','$username','$md5','Tidak Ada','$location','Sema & Dema', '$status');
insert into semadema values(' ','$nama', '$location','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','Belum Diisi','$username')";
$stmti = $db->prepare($insert);
$stmti->setFetchMode(PDO::FETCH_OBJ);
$stmti->execute();
...
echo '<meta http-equiv="Refresh" content="1; URL=usersemadema.php">';
           ...
           echo '<script type="text/javascript">';
  echo 'setTimeout(function () { swal("Gagal!","Nama sema & dema telah ada!","warning");})</script>';
echo '<meta http-equiv="Refresh" content="2; URL=usersemadema.php">';
        } else {
            echo '<script type="text/javascript">';
  echo 'setTimeout(function () { swal("Gagal!","Username telah ada!","error");})</script>';
echo '<meta http-equiv="Refresh" content="2; URL=usersemadema.php">';
        }
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            ...
            <div class="row">
				...
				foreach ($data as $value) {
					...
					<td>
						<a href="editsema.php?id=<?php echo $value ['id'] ?>" class="btn btn-default btn-sm btn-icon icon-left">
							<i class="fa fa-pencil"></i>
							Edit
						</a>


						<a href="hapus-sema.php?id=<?php echo $value['id']?>" onclick="return hapus()" class="btn btn-danger btn-sm btn-icon icon-left">
