...
if(!isset($_SESSION["username"])){
    ...
    $query = $db->prepare("SELECT * FROM proker, user where proker.oleh=user.username and user.program_studi='$z'");
...
<?php 
    ...
    // Buat prepared statement untuk mengambil semua data dari tbBiodata
    $query = $db->prepare("SELECT * FROM proker, user where proker.oleh=user.username and user.program_studi='$z' and user.keterangan='Himpunan'");
    // Jalankan perintah SQL
    $query->execute();
    // Ambil semua data dan masukkan ke variable $data
    $data = $query->fetchAll();
    ?>
                ...
                foreach ($data as $value) {
                    ...
                    <td><a href="download.php?file=<?php echo $value['file'] ?>" class="btn btn-xs btn btn-primary" >Download PDF</a></td>
                    <td>  <p><span class="badge badge-<?php
            ...
            ?>">
                 ...
                 ?>
                    ...
                    <?php if ($value['status_proker']=="Telah Disetujui"){ ?>
                        ...
                        <button type="button" class="btn-sm btn btn-warning"  data-toggle="popover" data-placement="auto top" data-content="LPJ Belum Di Upload">
    ...
    // Buat prepared statement untuk mengambil semua data dari tbBiodata
    $query = $db->prepare("SELECT * FROM proker, user where proker.oleh=user.username and user.program_studi='$z' and user.keterangan='Sema & Dema'");
    // Jalankan perintah SQL
    $query->execute();
    // Ambil semua data dan masukkan ke variable $data
    $data = $query->fetchAll();
    ...
    <?php
    $id_proker=$value['id_proker'];
    $query = $db->prepare("SELECT SUM(persentase) as total_sema from detail_proker where status='Telah Tercapai' and bukti NOT IN ('Belum Dikirim') and id_proker='$id_proker'");
    $query->execute();
