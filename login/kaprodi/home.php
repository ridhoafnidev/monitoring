...
<body>
    ...
    foreach ($data3 as $value) {
       $t = $value['program_studi'];
    }
   
    // Buat prepared statement untuk mengambil semua data dari tbBiodata
    $query = $db->prepare("SELECT count(*) as proker_tunggu from proker,user where proker.oleh=user.username and status_proker='Menunggu' and user.program_studi='$t'");
...
?>
                ...
                foreach ($data2 as $value) {
                    $no++;
                                             ?> <?php echo $value['proker_tunggu'] ?><?php } ?></h2>
                            <span> Program Kerja Menunggu Persetujuan Anda </span>
                        </div>
                        </a>
                    </div>
                </div>
